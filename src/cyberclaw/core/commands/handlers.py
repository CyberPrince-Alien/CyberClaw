...
from cyberclaw.core.commands.base import Command
from cyberclaw.channel.access import ChannelAccessManager
from cyberclaw.utils.def_loader import DefNotFoundError
...
class PairingCommand(Command):
    """Manage channel pairing approvals."""

    name = "pairing"
    description = "Approve or list channel pairing requests"

    async def execute(self, args: str, session: "AgentSession") -> str:
        parts = args.strip().split()
        manager = ChannelAccessManager(session.shared_context.config)

        if not parts or parts[0] == "list":
            pairings = manager.list_pairings()
            pending = pairings.get("pending", {})
            approved = pairings.get("approved", {})
            ...
            if pending:
                ...
                for code, record in pending.items():
                    lines.append(
                        f"- `{code}` {record.get('platform')}: `{record.get('source')}`"
                    )
            else:
                lines.append("\n**Pending:** none")
            ...
            if approved:
                lines.append("\n**Approved:**")
                for platform, sources in approved.items():
        ...
        if len(parts) == 3 and parts[0] == "approve":
            ...
            try:
                source = manager.approve(platform, code)
            except ValueError as e:
                return f"Approval failed: {e}"
            return f"Approved `{source}` for `{platform}`."

        return (
            "**Usage:** `/pairing list` or "
