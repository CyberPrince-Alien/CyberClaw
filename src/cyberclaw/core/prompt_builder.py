...
class PromptBuilder:
    ...
    def _build_operational_guardrails(self) -> str:
        ...
        return (
            ...
            "## Fallback Tool Protocol\n"
            "If you cannot call tools natively, or if the API does not support it, you can call any tool by outputting a JSON code block in your response. The block MUST match the following format:\n"
            "```json\n"
            "{\n"
            "  \"name\": \"tool_name\",\n"
            "  \"arguments\": {\n"
            ...
            "}\n"
            "```\n"
            "Only output ONE tool call at a time. The system will execute it and provide the results in the next turn.\n\n"
